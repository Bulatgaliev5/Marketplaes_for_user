using yoomoney_api.authorize;


namespace Marketplaes02.BD
{
    public class Yoomoney
    {
        public string authorizevar;

        Authorize authorize;
        public async void LoadData()
        {
            //Marketplaes02

            var clientIdvar = "56436F1B322FEB2269D708DBF543ACDCA962C113823AE7808604B4B3B9E68DC4";
            var redirectUrivar = "https://t.me/Bulatgaliev53";


            authorize = new(clientId: clientIdvar, redirectUri: redirectUrivar, scope: new[]
            {

                    "account-info",
                     "operation-history",
                       "operation-details",
                          "incoming-transfers",
                             "payment-p2p",

             });

            authorizevar = authorize.AuthorizeUrl;
            var YOUR_СODEvar = "https://t.me/Bulatgaliev53?code=";
            var token = await authorize.GetAccessToken(code: YOUR_СODEvar, clientId: clientIdvar, redirectUri: redirectUrivar);



        }


        public async void LoadDataAccaunt()
        {
            //  var YOUR_TOKENvar = "410015744747795.";
            //   var client = new Client(token: YOUR_TOKENvar);
            // var  accountInfo = client.GetAccountInfo(token: YOUR_TOKENvar);

            //   var operationHistory = client.GetOperationHistory(token: YOUR_TOKENvar);
            // accountInfo.Print();


        }
    }
}
